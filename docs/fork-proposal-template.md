...
Canonical template. Copy into a consuming repo as `.ozark/fork-proposals/<dep>.md`. Every field below is required; leave none blank.

A coding agent writing this proposal MUST NOT modify any manifest file (`package.json`, `Cargo.toml`, `go.mod`) or lockfile. The proposal IS the agent's only change. The maintainer runs the fork-and-trim workflow and opens any follow-up consumer PRs.
